...
void createT3RegressionChannel() {
   ...
   createRegressionChannel(T3_REGRESSION_CHANNEL, t3p1DateTime, t3EndDatetime, InpT3RegressionChannelColor, true);
}
...
void createT3RegressionChannelLevels() {
    ...
    double   t3RegressionChannel0StartValue   = ObjectGetValueByTime(ChartID(), T3_REGRESSION_CHANNEL, t3p1DateTime, (t3trendDirection == TREND_DIRECTION_LONG) ? 1 : 2);
    int      t3RegressionChannel0StartCandle = iBarShift(Symbol(), Period(), t3p1DateTime);
    ...
    createTrendLine(T3_REGRESSION_CHANNEL + "0", t3p1DateTime, t3RegressionChannel0StartValue, iTime(Symbol(), PERIOD_CURRENT, 0), t3RegressionChannel0EndTrendlineLevel, InpT3FiboLevelsColor, 2, STYLE_SOLID, " 0%");

    double   t3RegressionChannel100StartValue   = ObjectGetValueByTime(ChartID(), T3_REGRESSION_CHANNEL, t3p1DateTime, (t3trendDirection == TREND_DIRECTION_LONG) ? 2 : 1);
    int      t3RegressionChannel100StartCandle = iBarShift(Symbol(), Period(), t3p1DateTime);
    ...
    createTrendLine(T3_REGRESSION_CHANNEL + "100", t3p1DateTime, t3RegressionChannel100StartValue, iTime(Symbol(), PERIOD_CURRENT, 0), t3RegressionChannel100EndTrendlineLevel, InpT3FiboLevelsColor, 2, STYLE_SOLID, " 100%");
   ...
   for( int t3FiboLevelsId = 0; t3FiboLevelsId < ArraySize(t3FiboLevelsArray); t3FiboLevelsId++) {
      ...
      if(level == InpT3AlertOnRegressionChannelLevel) {
        lineWidth = 1;
        style = STYLE_DASH;
      ...
      createTrendLine(T3_REGRESSION_CHANNEL + DoubleToString(level, 1), t3p1DateTime, t3TrendlineStartValue, iTime(Symbol(), PERIOD_CURRENT, 0), t3TrendlineEndValue, levelColor, lineWidth, style, " " + DoubleToString(level, 1) + "%");
   }
}
