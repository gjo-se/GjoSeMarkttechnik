...
void createT3StopLossTrendline(double pStopLossLevel) {
   ...
   double offsetTrendline = (t3trendDirection == TREND_DIRECTION_LONG) ? -(InpT3StopLoss * 0.1) : (InpT3StopLoss * 0.1);

   createTrendLine(T3_STOP_LOSS_TLINE, iTime(Symbol(), Period(), barShift), pStopLossLevel + (offsetTrendline * Point()), iTime(Symbol(), Period(), 0), pStopLossLevel + (offsetTrendline * Point()), InpT3TrendLineColor, 2, STYLE_SOLID, T3_STOP_LOSS_TLINE, rayLeft, rayRight, zOrder, isBackground, isSelected, isSelectable);
}
